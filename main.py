...
for day in range(1, 8):
    ...
    while True:
        ...
        if score == -1 or (0 <= score <= 100):
            break
        print("Score must be between 0 and 100, or -1 for absent.")
...
if attempted_days > 0:
    print(f"Highest Score          : Day {highest_score_day} ({highest_score})")
    print(f"Lowest Score           : Day {lowest_score_day} ({lowest_score})")

# TODO: Display first critical details only when
# a critical score exists.
if critical_score_found:
    print(f"First Critical Score   : Day {first_critical_day} ({first_critical_score})")
